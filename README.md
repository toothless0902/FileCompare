...
- 구현한 내용 (위 그림 참조)
    - 중앙의 복사 버튼(>>> , <<<)을 통해 선택된 파일을 건너편 폴더로 물리적으로 복사하는 File.Copy 기능을 구현했습니다.
    - 파일 복사 수행 전 대상 경로의 존재 여부와 수정 시간을 전수 조사하여, 덮어쓰기 시 데이터 손실 위험을 사전에 계산합니다.
    - 특히 원본이 대상보다 오래된 파일(Old)일 경우에만 확인 대화상자를 띄워 사용자의 최종 승인을 받는 지능형 조건부 복사 시스템을 적용했습니다.
    - 작업 완료 후 리스트를 자동 갱신하여 복사된 파일이 동일 상태(검은색)로 변하는 과정을 실시간 피드백합니다.
...
- 구현한 내용 (위 그림 참조)
    - 단일 폴더 검색을 넘어 EnumerateDirectories를 이용한 하위 디렉터리 탐색 기능을 추가하여 복잡한 폴더 구조를 관리할 수 있게 했습니다.
    - 하위 폴더는 리스트 상에서 <DIR> 태그와 함께 별도로 표시하여 시각적 구분을 명확히 했습니다.
    - 폴더 복사 명령 시 내부의 모든 파일과 서브 디렉터리를 포함하여 복사하는 프로세스를 구축하여 대량 데이터 백업 기능을 완성했습니다.
    - 복잡한 경로 합산 시 발생할 수 있는 오류를 방지하기 위해 Path.Combine 메서드를 사용하여 안정적인 경로 연산 로직을 구현했습니다.
    - 하위 폴더 복사 시 발생할 수 있는 경로 길이 초과나 접근 권한 오류를 방어하기 위해 try-catch 예외 처리 구문을 강화했습니다.